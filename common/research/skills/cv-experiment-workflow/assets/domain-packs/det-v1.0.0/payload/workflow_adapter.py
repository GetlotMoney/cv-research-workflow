...
def _runtime(project: _Path) -> _types.ModuleType:
    ...
    source = root / "src" / "det" / "runtime.py"
    _regular_file(source, "PACK-DET runtime")
    name = f"_pack_det_runtime_{_uuid.uuid4().hex}"
    spec = _importlib_util.spec_from_file_location(name, source)
    if spec is None or spec.loader is None:
        raise ValueError("无法加载 PACK-DET runtime")
    ...
    *,
    ...
    if mode == "synthetic_debug":
        ...
        if (
            ...
            raise ValueError(
                "config.metric_definition 与检测模板固定指标口径不一致"
            )
    else:
...
def _bound_contract(
    ...
    if (
        ...
        raise ValueError("Run frozen.code 中央声明与 PACK-DET V1 不一致")
    if not isinstance(environment, dict):
...
def _identity(run: dict[str, _Any]) -> dict[str, _Any]:
    ...
    expected_code = {"template_id": "PACK-DET", "version": "1.0.0"}
...
def inspect(project: _Path) -> dict[str, _Any]:
    return {
        ...
        "direction": "det",
        "template_id": "PACK-DET",
        "template_version": "1.0.0",
        "standard": {"debug_required": True},
        "model": "tiny_fixed_query_detector",
        "synthetic": {
...
def validate(project: _Path) -> list[str]:
    ...
    for relative in (
        ...
        if not (root / relative).is_file():
            issues.append(f"缺少目标检测模板文件：{relative}")
    try:
    ...
    if version != "2.0.11":
        issues.append(
            "OPTIONAL_NOT_INSTALLED：正式 COCO bbox 评估需要 "
...
def prepare_runs(
    ...
    if mode == "synthetic_debug":
        ...
        annotation = _resolve_local_path(
            project,
            config["annotation_file"],
            "annotation_file",
        )
...
def execute(
    ...
    try:
        ...
        if _os.path.lexists(staging):
            raise RuntimeError(
                "PACK-DET 运行失败；随机 staging 不会递归删除，"
