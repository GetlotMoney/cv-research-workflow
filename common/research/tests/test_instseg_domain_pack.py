...
from workflow_core.domain_packs import resolve_domain_pack, validate_domain_pack  # noqa: E402
...
INSTSEG_FORMAL_METRIC_DEFINITION = {
    "coco_segm_ap": (
...
class InstanceSegmentationDomainPackTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory()
        self.addCleanup(self.temporary.cleanup)
        self.root = Path(self.temporary.name)

    def _create_repository(self, name: str = "instance-segmenter") -> Path:
        ...
        self.assertEqual("PACK-INSTSEG", created["repository"]["template_id"])
    ...
    def _adapter_run(
        ...
        *,
        ...
        frozen_data = data or {
            ...
            "evaluation": INSTSEG_DEBUG_EVALUATION,
        }
        return {
            "id": run_id,
            "purpose": purpose,
    ...
    def _write_coco(self, root: Path) -> tuple[Path, Path]:
        ...
        Image.new("RGB", (16, 16), (20, 60, 120)).save(images / "one.png")
    ...
    def test_formal_prepare_injects_fixed_metric_definitions(self) -> None:
        repository = self._create_repository("instseg-metric-definition")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_metric_definition_adapter",
        )
        images, annotation = self._write_coco(
            self.root / "instseg-metric-data"
        )
        config = {
            "mode": "local_coco",
            "device": "cpu",
            ...
            "dataset_id": "tiny-coco-instseg-metric",
            "version": "2026.07",
            "source_uri": (
                "https://example.org/datasets/tiny-coco-instseg-metric"
            ),
            "split": "val",
        }
        ...
        for mutation in ("missing", "changed", "extra"):
            with self.subTest(mutation=mutation):
                ...
                if mutation == "missing":
                    definitions.pop("coco_segm_ap")
                elif mutation == "changed":
                    definitions["coco_segm_ap"] = "user-defined score"
                else:
    ...
    def test_manifest_is_independent_and_declares_fixed_sources(self) -> None:
        pack = resolve_domain_pack("instseg")
        manifest = validate_domain_pack(pack)
        self.assertEqual("PACK-INSTSEG", manifest["template_id"])
        ...
        self.assertTrue(
            {
                ...
                "src/instseg/smoke.py",
                "workflow_adapter.py",
            }.issubset(files)
        )
        python = "\n".join(
            path.read_text(encoding="utf-8")
            for path in (pack / "payload").rglob("*.py")
        )
        self.assertNotIn("src.det", python)
        self.assertNotIn("_shared", python)
        downloads = read_json(pack / "payload" / "DOWNLOADS.json")
    ...
    def test_synthetic_chain_writes_real_png_masks_and_debug_metrics_only(self) -> None:
        repository = self._create_repository()
        train = self.root / "instseg-train"
        evaluation = self.root / "instseg-eval"
        inference = self.root / "instseg-infer"
        self._run(
            repository,
            "-m",
            "src.instseg.train",
        ...
        record = read_json(train / "run.json")
        self.assertGreater(record["optimizer_steps"], 0)
        self.assertTrue(record["checkpoint_reloaded"])
        ...
        self.assertEqual(
            {"debug_mask_mean_iou", "debug_precision_at_mask_iou_0_5"},
            set(metrics),
        )
        self.assertTrue(
            all(math.isfinite(value) and 0.0 <= value <= 1.0 for value in metrics.values())
        )
        predictions = read_json(inference / "predictions.json")["predictions"]
        self.assertGreater(len(predictions), 0)
        for item in predictions:
            input_path = inference / item["input_path"]
            mask_path = inference / item["mask_path"]
            self.assertEqual(
                f"sha256:{hashlib.sha256(input_path.read_bytes()).hexdigest()}",
                item["input_sha256"],
            )
            self.assertEqual(
                f"sha256:{hashlib.sha256(mask_path.read_bytes()).hexdigest()}",
                item["mask_sha256"],
            )
            with Image.open(mask_path) as mask:
                self.assertEqual("L", mask.mode)
                self.assertIn(255, set(mask.get_flattened_data()))

    def test_device_contract_keeps_synthetic_cpu_and_cuda_fails_before_outputs(
        self,
    ) -> None:
        repository = self.root / "instseg-device-repo"
        shutil.copytree(
        ...
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_device_adapter",
        )
        runtime = self._load_python(
            repository / "src" / "instseg" / "runtime.py",
            "_instseg_device_runtime",
        )
        ...
        images, annotation = self._write_coco(self.root / "instseg-device-data")
        formal_config = {
            "mode": "local_coco",
            "device": "cuda",
            "data_root": str(images),
            "annotation_file": str(annotation),
            "dataset_id": "tiny-coco-instseg-device",
            "version": "2026.07",
            "source_uri": "https://example.org/datasets/tiny-coco-instseg-device",
            "split": "val",
        }
        variant = adapter.prepare_runs(
            repository,
            {"route_inputs": {"config": formal_config, "seed": 23}},
        )[0]
        ...
        with mock.patch.object(torch.cuda, "is_available", return_value=False):
            ...
            direct_output = self.root / "instseg-cuda-unavailable"
            with self.assertRaisesRegex(RuntimeError, "CUDA|cuda"):
            ...
            evaluation_output = self.root / "instseg-cuda-evaluation"
            inference_output = self.root / "instseg-cuda-inference"
    ...
    def test_cuda_probe_failure_stops_before_staging(self) -> None:
        repository = self.root / "instseg-cuda-probe-repo"
        shutil.copytree(
            Path(__file__).resolve().parents[1]
            / "skills"
            / "cv-experiment-workflow"
        ...
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_cuda_probe_adapter",
        )
        images, annotation = self._write_coco(self.root / "instseg-probe-data")
        variant = adapter.prepare_runs(
            repository,
            {
                "route_inputs": {
                    "config": {
                        ...
                        "data_root": str(images),
                        "annotation_file": str(annotation),
                        "dataset_id": "tiny-coco-instseg-probe",
                        "version": "2026.07",
                        "source_uri": "https://example.org/instseg-probe",
                        "split": "val",
                    },
                    "seed": 23,
                },
        ...
        probe.assert_called_once_with("cuda", seed=23)
        self.assertFalse((repository / ".cv-workflow-output").exists())

    def test_cuda_probe_uses_real_mm_and_preserves_cuda_environment(self) -> None:
        repository = self.root / "instseg-cuda-env-repo"
        shutil.copytree(
            Path(__file__).resolve().parents[1]
            / "skills"
            / "cv-experiment-workflow"
        ...
        with (
            ...
            with self.assertRaisesRegex(RuntimeError, "CUDA|cuda"):
                adapter._probe_cuda("cuda", seed=23)
    ...
    def test_adapter_has_evaluation_identity_and_rejects_synthetic_evidence(self) -> None:
        repository = self._create_repository("instseg-adapter")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_adapter",
        )
        ...
        self.assertEqual(FROZEN_FIELDS, set(prepared))
        self.assertIsInstance(prepared["data"]["evaluation"], dict)
        self.assertEqual("synthetic_debug_only", prepared["data"]["kind"])
        self.assertEqual("segm", prepared["data"]["evaluation"]["iou_type"])
        self.assertEqual(
            "debug_mask_metrics",
            prepared["data"]["evaluation"]["backend"],
        )
        self.assertNotIn("run_kind", prepared)
        self.assertNotIn("paper_eligible", prepared)
        with self.assertRaisesRegex(ValueError, "合成|evidence|论文"):
            adapter.execute(
                repository,
                "start",
                self._adapter_run("RUN-0001", purpose="evidence"),
            )
        ...
        self.assertTrue(
            any(path.endswith(".png") for path in parsed["artifacts"]),
            parsed["artifacts"],
        )

    def test_adapter_identity_requires_exact_frozen_code_environment_and_purpose(self) -> None:
        repository = self._create_repository("instseg-frozen-contract")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_frozen_contract_adapter",
        )
    ...
    def test_polygon_contract_and_pycocotools_prerequisite_are_strict(self) -> None:
        repository = self._create_repository("instseg-coco")
        runtime = self._load_python(
            repository / "src" / "instseg" / "runtime.py",
            "_instseg_runtime",
        )
        images, annotation = self._write_coco(self.root / "instseg-coco-data")
        valid = runtime.load_coco_dataset(images, annotation)
        self.assertEqual(1, len(valid["annotations"]))
        ...
        for index, (field, value) in enumerate(strict_integer_cases):
            ...
            attacked["annotations"][0][field] = value
            candidate = annotation.with_name(f"strict-integer-{index}.json")
            candidate.write_text(json.dumps(attacked), encoding="utf-8")
        ...
        with mock.patch.object(runtime, "MAX_TOTAL_PIXELS", 128, create=True):
            with self.assertRaisesRegex(ValueError, "像素|pixel|上限"):
                runtime.load_coco_dataset(images, annotation)
        with mock.patch.object(runtime, "MAX_TOTAL_MASK_PIXELS", 128, create=True):
            with self.assertRaisesRegex(ValueError, "mask|掩码|像素|上限"):
                runtime.load_coco_dataset(images, annotation)
        ...
        invalid_cases = (
            ...
            {"iscrowd": 1},
            {"segmentation": [[2.0, 2.0, 3.0, 3.0]]},
            {"segmentation": [[2.0, 2.0, 20.0, 2.0, 10.0, 10.0]]},
            {"segmentation": [[2.0, 2.0, 4.0, 4.0, 6.0, 6.0]]},
        )
        for index, patch in enumerate(invalid_cases):
            attacked = json.loads(json.dumps(original))
            attacked["annotations"][0].update(patch)
            candidate = annotation.with_name(f"invalid-{index}.json")
            candidate.write_text(json.dumps(attacked), encoding="utf-8")
            with self.subTest(patch=patch), self.assertRaisesRegex(
                ValueError,
                "polygon|segmentation|iscrowd|多边形|越界|退化",
            ):
                runtime.load_coco_dataset(images, candidate)

        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_coco_adapter",
        )
        config = {
            "mode": "local_coco",
            "device": "cpu",
            "data_root": str(images),
            "annotation_file": str(annotation),
            "dataset_id": "tiny-coco-instseg",
            "version": "2026.07",
            "source_uri": "https://example.org/datasets/tiny-coco-instseg",
            "split": "val",
        }
        prepared = adapter.prepare_runs(
            repository,
    ...
    def test_links_read_time_replacement_and_output_tampering_are_rejected(self) -> None:
        repository = self._create_repository("instseg-security")
        runtime = self._load_python(
            repository / "src" / "instseg" / "runtime.py",
            "_instseg_security_runtime",
        )
        images, annotation = self._write_coco(self.root / "instseg-security-data")
        linked = self.root / "linked-instseg-annotation.json"
        try:
            linked.symlink_to(annotation)
        except OSError:
            linked = None
        if linked is not None:
            with self.assertRaisesRegex(ValueError, "link|reparse"):
                runtime.load_coco_dataset(images, linked)

        victim = images / "one.png"
        replacement = images / "replacement.png"
        Image.new("RGB", (16, 16), (9, 8, 7)).save(replacement)
        ...
        prediction["predictions"][0]["mask_sha256"] = "sha256:" + ("f" * 64)
    ...
    def test_bad_checkpoint_is_rejected_and_outputs_are_never_overwritten(self) -> None:
        repository = self._create_repository("instseg-checkpoint")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_checkpoint_adapter",
        )
        run = self._adapter_run("RUN-0005")
        adapter.execute(repository, "start", run)
        ...
        checkpoint["model_config"]["num_queries"] += 1
        checkpoint_path.unlink()
        torch.save(checkpoint, checkpoint_path)
        with self.assertRaisesRegex(ValueError, "checkpoint|身份|state_dict"):
            adapter.parse_result(repository, run)

    def test_checkpoint_uses_one_stable_byte_snapshot(self) -> None:
        repository = self._create_repository("instseg-checkpoint-snapshot")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_checkpoint_snapshot_adapter",
        )
        ...
        runtime = self._load_python(
            repository / "src" / "instseg" / "runtime.py",
            "_instseg_checkpoint_snapshot_runtime",
        )
    ...
    def test_output_tree_and_json_top_levels_are_exact_and_resource_bounded(self) -> None:
        repository = self._create_repository("instseg-output-exact")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_output_exact_adapter",
        )
        ...
        with mock.patch.object(runtime, "MAX_TOTAL_MASK_PIXELS", 1, create=True):
            with self.assertRaisesRegex(ValueError, "mask|掩码|像素|上限"):
                runtime.validate_workflow_output(
                    output,
                    mode="synthetic_debug",
    ...
    def test_output_total_bytes_include_png_tails_and_every_artifact(self) -> None:
        repository = self._create_repository("instseg-output-total-bytes")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_output_total_bytes_adapter",
        )
        ...
        runtime = self._load_python(
            repository / "src" / "instseg" / "runtime.py",
            "_instseg_output_total_bytes_runtime",
        )
        ...
        for item in predictions["predictions"]:
            for path_field, hash_field in (
                ...
                artifact_path = output / item[path_field]
                artifact_path.write_bytes(artifact_path.read_bytes() + padding)
                item[hash_field] = (
                    "sha256:"
    ...
    def test_formal_segm_sidecar_is_self_contained_for_exact_backend(self) -> None:
        repository = self._create_repository("instseg-formal-sidecar")
        runtime = self._load_python(
            repository / "src" / "instseg" / "runtime.py",
            "_instseg_formal_sidecar_runtime",
        )
        images, annotation = self._write_coco(self.root / "instseg-formal-data")
        dataset = runtime.load_coco_dataset(images, annotation)
        output = self.root / "instseg-formal-output"
        output.mkdir()
        ...
        class FakeMaskUtils:
            @staticmethod
            def encode(array: object) -> dict[str, object]:
                return {"size": [16, 16], "counts": b"encoded"}

        runtime.require_pycocotools = lambda: (
            FakeCOCO,
            FakeEvaluator,
            FakeMaskUtils,
        )
        image = dataset["images"][0]
        annotation_record = image["annotations"][0]
        predictions = [
            {
                "image_id": image["id"],
                "category_id": annotation_record["category_id"],
                "score": 0.5,
            }
        ]
        metrics = runtime._formal_metrics(
            output,
            dataset,
            predictions,
            [annotation_record["mask"]],
        )
        self.assertEqual("segm", observed["iou_type"])
        self.assertEqual(
            {"coco_segm_ap": 0.1, "coco_segm_ap50": 0.2, "coco_segm_ap75": 0.3},
            metrics,
        )

    def test_output_manifest_rejects_windows_backslash_traversal(self) -> None:
        repository = self._create_repository("instseg-output-traversal")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_output_traversal_adapter",
        )
    ...
    def test_prediction_identity_rejects_duplicate_images(self) -> None:
        repository = self._create_repository("instseg-duplicate-prediction")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_duplicate_prediction_adapter",
        )
    ...
    def test_prediction_identity_is_bound_to_frozen_manifest_and_categories(self) -> None:
        repository = self._create_repository("instseg-frozen-prediction")
        adapter = self._load_python(
            repository / "workflow_adapter.py",
            "_instseg_frozen_prediction_adapter",
        )
    ...
    def test_huge_config_and_checkpoint_integers_fail_before_allocation(self) -> None:
        repository = self._create_repository("instseg-bounded-model")
        runtime = self._load_python(
            repository / "src" / "instseg" / "runtime.py",
            "_instseg_bounded_model_runtime",
        )
        ...
        for field in (
            ...
            candidate = self.root / f"instseg-huge-{field}.json"
            ...
            with mock.patch.object(
                ...
                runtime.run_training(
                    candidate,
                    self.root / f"instseg-huge-{field}-output",
                    mode="synthetic_debug",
                )

        valid_output = self.root / "instseg-small-checkpoint"
        runtime.run_training(
        ...
        for field in ("num_classes", "num_queries", "image_size"):
            ...
            candidate = self.root / f"instseg-huge-checkpoint-{field}.pt"
